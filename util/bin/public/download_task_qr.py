...
def main():
  ...
  if tempfile:
    url = 'http://'+os.environ['HTTP_HOST']+'/tasks/'+uid+'/temp/'+task+'/'+filetype
  else:
    url = 'http://'+os.environ['HTTP_HOST']+'/tasks/'+uid+'/'+task+'/'+filetype

 
#  stdout = subprocess.check_output(['qrencode', '-l', 'M', '-o', '-', url])
  process = subprocess.Popen(['qrencode', '-l', 'M', '-o', '-', url], stdout=subprocess.PIPE)
